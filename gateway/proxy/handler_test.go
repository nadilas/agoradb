...
func (s *assertingService) PingEmpty(ctx context.Context, _ *Empty) (*PingResponse, error) {
...
func (s *assertingService) Ping(ctx context.Context, ping *PingRequest) (*PingResponse, error) {
...
func (s *assertingService) PingError(ctx context.Context, ping *PingRequest) (*Empty, error) {
	return nil, grpc.Errorf(codes.FailedPrecondition, "Userspace error.")
}

func (s *assertingService) PingList(ping *PingRequest, stream TestService_PingListServer) error {
	// Send user trailers and headers.
...
func (s *assertingService) PingStream(stream TestService_PingStreamServer) error {
	stream.SendHeader(metadata.Pairs(serverHeaderMdKey, "I like turtles."))
	counter := int32(0)
	for {
		...
		pong := &PingResponse{Value: ping.Value, Counter: counter}
...
func (s *ProxyHappySuite) TestPingEmptyCarriesClientMetadata() {
	...
	out, err := s.testClient.PingEmpty(ctx, &Empty{})
	require.NoError(s.T(), err, "PingEmpty should succeed without errors")
	require.Equal(s.T(), &PingResponse{Value: pingDefaultValue, Counter: 42}, out)
}
...
func (s *ProxyHappySuite) TestPingCarriesServerHeadersAndTrailers() {
	...
	out, err := s.testClient.Ping(s.ctx(), &PingRequest{Value: "foo"}, grpc.Header(&headerMd), grpc.Trailer(&trailerMd))
	require.NoError(s.T(), err, "Ping should succeed without errors")
	require.Equal(s.T(), &PingResponse{Value: "foo", Counter: 42}, out)
...
func (s *ProxyHappySuite) TestPingErrorPropagatesAppError() {
	_, err := s.testClient.PingError(s.ctx(), &PingRequest{Value: "foo"})
...
func (s *ProxyHappySuite) TestDirectorErrorIsPropagated() {
	...
	_, err := s.testClient.Ping(ctx, &PingRequest{Value: "foo"})
...
func (s *ProxyHappySuite) TestPingStream_FullDuplexWorks() {
	...
	for i := 0; i < countListResponses; i++ {
		ping := &PingRequest{Value: fmt.Sprintf("foo:%d", i)}
...
func (s *ProxyHappySuite) SetupSuite() {
	...
	RegisterTestServiceServer(s.server, &assertingService{t: s.T()})

	// Setup of the proxy's Director.
	s.serverClientConn, err = grpc.Dial(s.serverListener.Addr().String(), grpc.WithInsecure(), grpc.WithDefaultCallOptions(grpc.ForceCodec(Codec())))
	...
	require.NoError(s.T(), err, "must not error on deferred client Dial")
	s.testClient = NewTestServiceClient(clientConn)
}
