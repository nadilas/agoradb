...
Modified to access gateway in order to support reflection on all database services as well
...
func registerReflection(g *gatewayServer) {
	rpb.RegisterServerReflectionServer(g.grpcServer, &serverReflectionServer{
		s: g.grpcServer,
		g: g,
	})
}
...
func (s *serverReflectionServer) getSymbols() (svcNames []string, symbolIndex map[string]*dpb.FileDescriptorProto) {
	// removed to gather database services live, not just once
	// s.initSymbols.Do(func() {
		serviceInfo := s.s.GetServiceInfo()
		for _, db := range s.g.serviceManager.AllDatabases() {
			for key, si := range db.Info {
				serviceInfo[key] = si
			}
		}
